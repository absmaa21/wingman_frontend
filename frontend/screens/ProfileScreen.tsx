...
import {loadAccountXP, loadFetchContent, loadPlayerMMR} from '../interface.ts';
...
import {getTitleByID} from "../helpers/ExtractFromGameContent.ts";
...
export default function ProfileScreen(
    ...
    const [user, setUser] = useState(api.getUserApi().getActiveUser())
    const puuid =
    ...
    const [playerLevel, setPlayerLevel] = useState(user?.accountXp?.Progress.Level ?? -1)
    const [playerTitle, setPlayerTitle] = useState(user ? getTitleByID(api, user.titleID) : '')
    ...
    useEffect(() => {
        ...
        if (canFetch) {
            ...
            loadPlayerMMR({api: api, puuid: puuid, setData: setProfileData});
    ...
    useEffect(() => {
        if (!user) {
            setUser(api.getUserApi().getActiveUser())
        }

        if (user && !playerTitle) {
            setPlayerTitle(getTitleByID(api, user.titleID))
        }
    ...
    return (
        <ScrollView
            ...
            {profileData && (
                <Column gap={4}>
                    <ProfileHeader
                        name={user?.accountInfo.acct.game_name ?? ''}
                        tag={user?.accountInfo.acct.tag_line ?? ''}
                        title={playerTitle ?? ''}
                        level={playerLevel}
                        cardUuid={user?.cardID ?? ''}
                    />
                    <Row>
                        <ProfileRank
